...
def predict_audio(audio_input):
    ...
    predicted_speech_class = np.argmax(cnn_probabilities)  # Get index of highest probability
    ...
    speech_emotion = class_labels[predicted_speech_class]  # Convert index to emotion label
    print(f"Speech Model Prediction: {speech_emotion}")

    return cnn_probabilities, speech_emotion, end_time - start_time, end_mem - start_mem, peak_mem
...
def predict_text(text):
    ...
    predicted_text_class = np.argmax(nb_probabilities) 
    ...
    text_emotion = class_labels[predicted_text_class]
    print(f"Text Model Prediction: {text_emotion}")

    return nb_probabilities, text_emotion, end_time - start_time, end_mem - start_mem, peak_mem

# ============================================================== FUSION MODEL =======================================================

def fusion_prediction(audio_input, text):
    # Get probabilities from both models
    cnn_probs, cnn_prediction, cnn_time, cnn_mem, cnn_peak = predict_audio(audio_input)
    nb_probs, text_prediction, nb_time, nb_mem, nb_peak = predict_text(text)
    ...
    return {
        "emotion_label": class_labels[final_prediction],
        "speech_prediction": cnn_prediction,
        "text_prediction": text_prediction,
        "confidence_scores": final_scores.tolist(),
        "cnn_time": cnn_time,
        "nb_time": nb_time,
        "cnn_mem": cnn_mem,
        "nb_mem": nb_mem,
...
def predict():
    ...
    result = fusion_prediction(audio_input, text)
